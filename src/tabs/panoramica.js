...
import { IF } from '../constants.js?v=20260716';
import { fE, fEk, fD } from '../utils/format.js?v=20260716';
import { dTS, cSt, bC } from '../utils/normalize.js?v=20260716';
import { $ } from '../utils/dom.js?v=20260716';
import { corsiForYear } from '../data/corsiForYear.js?v=20260716';
import { getRealSpend, getActiveCampaignCount } from '../data/helpers.js?v=20260716';
import { buildSuggestions, suggestCardHTML } from '../data/insights.js?v=20260716';
import { drawRevenuePerIntake, drawIntakeDonut } from '../charts/chartjs.js?v=20260716';
import { ZR_PCT_TARGET, ZR_MAX_DAYS } from '../data/thresholds.js?v=20260716';
...
export function renderPanoramica(corsi) {
  ...
  grid.innerHTML = sorted.map((c) => {
    ...
    return `<div class="corso-card ${isZR ? 'zona-rossa' : ''}">`
      ...
      + rl
      + (hasFunnel ? '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:8px 0 4px;padding-top:8px;border-top:1px solid var(--line-l,#e8e6e0)">'
        + fs.map((s, i) => {
          const w = Math.max(6, Math.round((s.v / fm) * 100));
          const p = i > 0 && fs[i - 1].v > 0 ? Math.round((s.v / fs[i - 1].v) * 100) + '%' : '';
          return `<div style="text-align:center"><div style="height:6px;background:var(--line-l,#e8e6e0);border-radius:3px;overflow:hidden;margin-bottom:4px"><div style="height:100%;width:${w}%;background:${s.c};border-radius:3px"></div></div><div style="font-family:var(--mono);font-size:13px;font-weight:800;color:var(--ink,#111)">${s.v}${p ? ' · ' + p : ''}</div><div style="font-size:9px;color:var(--ink-3,#7a7872);text-transform:uppercase;letter-spacing:.04em;font-weight:700">${s.l}</div></div>`;
        }).join('')
        + '</div>' : '')
      + '</div>';
  }).join('');
