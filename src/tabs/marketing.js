...
import { fE } from '../utils/format.js?v=20260716';
...
import { getRealSpend, attributeCampaignToYear } from '../data/helpers.js?v=20260716';
import { buildDailyChart } from '../charts/dailyChart.js?v=20260716';
...
export function renderMarketing() {
  ...
  // Year-aware spend — uses same attributeCampaignToYear as Panoramica and P&L
  ...
  mc.forEach((c) => {
    ...
    if (share > 0) {
      const ratio = spend > 0 ? share / spend : 0;
      metaSpend += share;
      metaLeads += Math.round((parseInt(c.leads) || 0) * ratio);
      metaClicks += Math.round((parseInt(c.clicks) || 0) * ratio);
      metaImpr += Math.round((parseInt(c.impressions) || 0) * ratio);
    }
  });
  ...
  gc.forEach((c) => {
    ...
    if (share > 0) {
      const ratio = spend > 0 ? share / spend : 0;
      gadsSpend += share;
      gadsClicks += Math.round((parseInt(c.clicks) || 0) * ratio);
      gadsConv += Math.round((parseFloat(c.conversions) || 0) * ratio);
    }
  });
